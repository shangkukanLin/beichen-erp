...
async function loadProjects() {
  ...
  try {
    ...
    for (const p of projects) {
      const drawings = await request.get<any, any>(`/dev/project/${p.id}/drawing`)
      p.drawingCount = (drawings || []).length
    }
...
<template>
  <div class="drawing-page">
    <el-card shadow="never" class="table-card">
      <template #header><span style="font-weight:600">项目图纸总览</span></template>
      ...
      <el-table :data="filteredProjects" border stripe v-loading="tableLoading">
        ...
        <el-table-column label="图纸数量" width="110" align="center">
          <template #default="{row}"><el-tag type="success" size="small">{{ row.drawingCount }}</el-tag></template>
        </el-table-column>
        <el-table-column prop="status" label="项目阶段" width="110" />
        <el-table-column label="操作" width="150" align="center">
          <template #default="{row}">
            <el-button type="primary" link @click="goToDrawing(row.id)">查看图纸</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
...
<style scoped>
.drawing-page { display:flex; flex-direction:column; gap:12px; }
