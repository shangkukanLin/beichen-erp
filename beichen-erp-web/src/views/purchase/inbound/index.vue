...
const form = reactive<PurchaseInbound>({
  orderId: undefined,
...
const items = ref<PurchaseInboundItem[]>([])

const detailVisible = ref(false)
const detailData = ref<PurchaseInbound>({})
const detailItems = ref<PurchaseInboundItem[]>([])
...
async function loadData() {
  ...
  try {
    ...
    const res = await getPurchaseInboundPage(params)
...
function resetForm() {
  Object.assign(form, { id: undefined, orderId: undefined, supplierId: undefined, warehouseId: undefined, inboundDate: '', remark: '' })
  items.value = []
}
function handleAdd() { resetForm(); dialogTitle.value = '新增采购入库'; dialogVisible.value = true; formRef.value?.clearValidate() }
async function handleEdit(row: PurchaseInbound) {
  resetForm(); Object.assign(form, row); dialogTitle.value = '编辑采购入库'; dialogVisible.value = true; formRef.value?.clearValidate()
...
async function handleSubmit() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const payload = { inbound: { ...form }, items: items.value }
      if (form.id) { await updatePurchaseInbound(form.id as number, payload); ElMessage.success('修改成功') }
      else { await createPurchaseInbound(payload); ElMessage.success('新增成功') }
...
async function handleAudit(row: PurchaseInbound) {
  try {
    await ElMessageBox.confirm(`确认审核采购入库「${row.code}」？审核后将增加库存并生成应付。`, '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
    await auditPurchaseInbound(row.id as number); ElMessage.success('审核成功，已增加库存并生成应付'); loadData()
  } catch { }
}
async function handleCancel(row: PurchaseInbound) {
  try {
    await ElMessageBox.confirm(`确认作废采购入库「${row.code}」？`, '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
    await cancelPurchaseInbound(row.id as number); ElMessage.success('已作废'); loadData()
  } catch { }
}
async function handleDetail(row: PurchaseInbound) {
  detailData.value = { ...row }
  try { const res = await getPurchaseInboundItems(row.id as number); detailItems.value = res || [] } catch { detailItems.value = [] }
  ...
  <div class="page">
    ...
    <el-card shadow="never" class="table-card">
      <el-table v-loading="tableLoading" :data="tableData" border stripe>
        ...
        <el-table-column prop="inboundDate" label="入库日期" width="120" align="center" />
    ...
    <el-drawer v-model="detailVisible" title="采购入库详情" size="60%">
