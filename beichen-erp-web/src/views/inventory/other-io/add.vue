...
async function loadDetail() {
  if (!editId) return
  try {
    const io = await request.get<any,any>(`/inventory/other/${editId}`)
    form.warehouseId = io.warehouseId; form.ioType = io.ioType
    form.ioDate = io.ioDate; form.remark = io.remark||''
    const its = await request.get<any,any>(`/inventory/other/${editId}/items`)
    if (Array.isArray(its)) items.value = its.map((i:any)=>({ materialId: i.materialId, materialName: i.materialName, spec: i.spec, unit: i.unit, quantity: i.quantity, remark: i.remark||'' }))
...
<template>
  <div style="display:flex;flex-direction:column;gap:12px">
    ...
    <div style="display:flex;gap:12px;justify-content:center"><el-button @click="router.push('/inventory/other-io')">取消</el-button><el-button type="primary" :loading="saving" @click="handleSubmit">保存</el-button></div>
  </div>
</template>
